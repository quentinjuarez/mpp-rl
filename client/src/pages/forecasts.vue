<template>
  <div class="space-y-4 pb-4">
    <h1 class="text-4xl font-bold">{{ $t('title.forecasts') }}</h1>

    <div v-if="loading || store.loading" class="text-center text-neutral-600 dark:text-neutral-400">
      {{ $t('common.loading') }}
    </div>
    ...
    <div v-else class="mx-auto flex flex-col items-center space-y-4">
      <!-- Calendar list -->
      <div
        class="sticky top-18 z-10 flex items-center space-x-2 bg-neutral-100 px-4 py-2 dark:bg-neutral-900"
      >
        ...
        <div class="flex max-w-[calc(100vw-128px)] space-x-2 overflow-auto md:max-w-[400px]">
          <button
            ...
            class="flex flex-col items-center gap-1 rounded px-2 py-1 transition-colors hover:bg-black/10 dark:hover:bg-white/10"
            :class="[{ '!bg-blue': isFocusedDay(day) }]"
          >
            <span class="text-xs text-neutral-300 dark:text-neutral-700">{{ getDay(day) }}</span>
      ...
      <div v-else class="text-center text-neutral-600 dark:text-neutral-400">
...
const focusMatches = computed(() => {
  ...
  const array = matches.value
    .filter((match) => {
      ...
      const hasForecast = store.forecasts?.some((f) => f.matchId === match.id)

      return hasForecast && day === match.begin_at.split('T')[0]
    })
    .sort((a, b) => {
