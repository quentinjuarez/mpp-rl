...
class UsersService {
  ...
  async getLeaderboard(serie_id?: number) {
    const res = await this.client.get<{ leaderboard: LeaderboardItem[] }>('/users/leaderboard', {
      params: {
        serie_id
      }
    })
  ...
  async updateUserPreference(preference: string, value: any) {
    return this.client.post(`/users/me/preferences`, { [preference]: value });
  }
}
