<template>
  <div class="w-fit rounded-lg p-1 transition-all duration-500 hover:bg-black/5 dark:hover:bg-white/5" :class="bgStyles">
    <div class="flex items-center gap-4 rounded-md bg-white/75 dark:bg-black/75 p-4 md:gap-8">
      <TeamLogo
        :team="blueTeam"
    ...
    <MatchDialog v-if="display" :match="props.match" @close="display = false" />
  </div>
</template>
...
function onClick(side: 'blue' | 'orange') {
  if (side === winner.value || notPlanned.value || !canForecast.value) return
...
const canForecast = computed(() => {
  return (
    ...
    Boolean(props.match.begin_at) &&
    new Date(props.match.begin_at) > new Date()
  )
})
